...
@section('content')
        ...
        <div class="row">
            <div class="col-12 col-md-6">
                <x-form.row>
                    ...
                    <x-form.input name="first_name" type="text" :value="$editedUser->first_name ?? null" />
                </x-form.row>
                <x-form.row>
                    <x-form.label for="last_name">{{ __('Last name') }}</x-form.label>
                    <x-form.input name="last_name" type="text" :value="$editedUser->last_name ?? null" />
                </x-form.row>
                <x-form.row>
                    <x-form.label for="email">{{ __('E-mail') }}</x-form.label>
                    <x-form.input name="email" type="email" :value="$editedUser->email ?? null" />
                </x-form.row>
                ...
                <x-form.row>
                    ...
                    <x-form.select name="status" :options="\App\Options\ActiveStatus::keysWithNames()" :value="$editedUser->status->value ?? null" />
                </x-form.row>
            </div>
            <div class="col-12 col-md-6">
                <x-form.row>
                    <x-form.label for="phone">{{ __('Phone number') }}</x-form.label>
                    <x-form.input name="phone" type="tel" :value="$editedUser->phone ?? null" />
                </x-form.row>
    ...
    <form method="POST" action="{{ route('users.destroy', $editedUser ?? '') }}">
        @csrf
        @method('DELETE')
